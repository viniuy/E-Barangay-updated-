...
export function AdminRequestManagement({ onNavigate }: AdminRequestManagementProps) {
  ...
  const submitAction = async () => {
    ...
    try {
      ...
      await updateRequestStatus(selectedRequest.id, status, currentUserId, remarks)
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Manage and review service requests from residents</p>
        </div>
                ...
                <Card key={request.id}>
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle>{request.item?.name || 'Unknown Service'}</CardTitle>
                        <CardDescription>
                          Requested by: {request.user?.username || request.user?.email || 'Unknown User'}
                        </CardDescription>
                      </div>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{request.item?.category?.name || 'N/A'}</p>
                      </div>
                      {request.reason && (
                        <div>
        ...
        {otherRequests.length > 0 && (
          <div>
            <h2 className="text-2xl font-semibold mb-4">All Requests ({otherRequests.length})</h2>
            <div className="grid gap-4">
                          ...
                          <div className="mt-2 space-y-1">
                            {request.actions.map((action: any, idx: number) => (
                              <div key={idx} className="text-sm text-muted-foreground border-l-2 pl-2">
                                ...
                                {action.remarks && <span> - {action.remarks}</span>}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedRequest && (
                <>
                  {actionType === 'approve' 
                    ? `Approve request for "${selectedRequest.item?.name || 'Unknown Service'}"?`
                    : `Reject request for "${selectedRequest.item?.name || 'Unknown Service'}"?`
                  }
                </>
              )}
            </DialogDescription>
